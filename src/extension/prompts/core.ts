...
export const ROLL20_INSTRUCTIONS = [
  ...
  "When asked to create a character or NPC, create a new character rather than modifying or substituting an existing one, even if its name or stat block matches. Reuse an existing character only when the game master requests it or when continuing or repairing the same creation task. Return the new character's actual ID and reuse that ID throughout the task; do not create duplicates on retries. A request to place more tokens for an existing creature does not itself require a new character sheet.",
  ...
  "Ordinary object properties such as avatar, name, and token imgsrc are synchronous reads: use character.get('avatar'), not await new Promise(resolve => character.get('avatar', resolve)). Passing a callback to a synchronous getter does not make it asynchronous and can leave the Promise unresolved. Use callback-based reads only for documented callback-only properties, and bound every such wait with a timeout.",
  "Budget the entire execute_roll20 call, not just individual waits: the extension stops waiting for a result after 45 seconds, although sandbox code may continue running. Leave a generous margin for API work and returning the result. Sequential per-property timeouts add up; 33 reads with 1.5-second timeouts can take 49.5 seconds. For large verification passes, use small batches of independent read-only checks with bounded concurrency and a shared deadline, returning partial results and explicitly naming unverified fields before time runs out. Split remaining checks across later calls. Do not parallelize dependent writes or automatically repeat mutations after a timeout; first inspect what completed. A Promise.race timeout does not cancel the underlying operation.",
].join(" ");
