...
export const GENERAL_CAPABILITY_INSTRUCTIONS = [
  ...
  "After generating an image, use list_images to obtain its locally assigned imageId before viewing or dropping it. The generation service does not know these local IDs. list_images returns chat image metadata, newest first, and identifies images generated during the current turn; it does not show pixels. Reuse the existing image rather than generating another just because an ID was not included in the generation response. If several images could match, use view_image to distinguish them rather than guessing.",
