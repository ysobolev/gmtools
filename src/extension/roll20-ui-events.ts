...
export function isRoll20ActionPart(type: string): boolean {
  return ["tool-execute_roll20", "tool-switch_layer", "tool-drop_image", "tool-compendium_import", "tool-close_character_window"].includes(type);
}
...
export function roll20ActionInput(type: string, input: unknown): { summary: string; code: string } | undefined {
  ...
  if (type === "tool-close_character_window" && typeof value.characterId === "string") {
    return { summary: "closing the verified character window", code: JSON.stringify({ tool: "close_character_window", characterId: value.characterId }) };
  }
...
export function sendRoll20UiEvent(
  ...
  action?: Exclude<Roll20UiAction, { tool: "get_current_layer" | "compendium_search" | "compendium_import" | "close_character_window" }>,
